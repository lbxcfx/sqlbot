...
def test_datasource_selection():
    ...
    for ds in datasources:
        print(f"  - ID: {ds['id']}, 名称: {ds['name']}, 描述: {ds['description']}, 类型: {ds['type']}")
    ...
    for ds in datasources:
        ...
        try:
            ...
            success, stdout, stderr = run_docker_sql(f"""
                ...
                FROM core_table t
                LEFT JOIN core_field f ON t.id = f.table_id
                WHERE t.ds_id = {ds['id']} AND t.checked = true
                ORDER BY t.table_name, f.field_index;
            """)
            ...
            if success:
                ...
                for line in lines:
                    if '|' in line:
                        parts = line.split('|')
                        if len(parts) >= 4:
                            table_name, table_comment, field_name, field_comment = parts[0].strip(), parts[1].strip(), parts[2].strip(), parts[3].strip()
                            if table_name:
...
def test_table_schema_generation(ds_id):
    ...
    # 获取数据源信息
    success, stdout, stderr = run_docker_sql(f"SELECT name, description, type FROM core_datasource WHERE id = {ds_id};")
    
    if not success:
        print(f"获取数据源信息失败: {stderr}")
        return None
    ...
    parts = lines[0].split('|')
    if len(parts) >= 3:
        name, description, ds_type = parts[0].strip(), parts[1].strip(), parts[2].strip()
    ...
    for line in lines:
        if '|' in line:
            parts = line.split('|')
            if len(parts) >= 6:
                table_name, table_comment, field_name, field_type, field_comment, checked = parts[0].strip(), parts[1].strip(), parts[2].strip(), parts[3].strip(), parts[4].strip(), parts[5].strip()
...
def main():
    ...
    # 2. 测试表结构生成
    schema_info = test_table_schema_generation(best_ds_id)
    ...
    if success:
        print("✓ 整个流程测试成功")
        print("\n问题分析:")
        print("数据库层面一切正常，问题可能在于:")
        print("1. 前端没有正确调用数据源选择API")
        print("2. 用户权限问题导致无法访问数据源")
        print("3. AI模型配置问题或API调用失败")
        print("4. 表结构嵌入(embedding)功能可能有问题")
    else:
        print("✗ 流程测试失败")
